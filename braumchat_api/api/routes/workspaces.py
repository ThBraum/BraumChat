...
from ...services.workspace_service import create_workspace, list_workspaces, get_workspace

router = APIRouter()

@router.post("/", response_model=WorkspaceRead)
async def create(payload: WorkspaceCreate, db: AsyncSession = Depends(get_db_dep), user=Depends(get_current_user)):
...
async def get_one(workspace_id: int, db: AsyncSession = Depends(get_db_dep), user=Depends(get_current_user)):
