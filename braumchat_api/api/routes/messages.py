...
async def get_messages(channel_id: int, limit: int = 50, db: AsyncSession = Depends(get_db_dep), user=Depends(get_current_user)):
...
async def post_message(channel_id: int, payload: MessageCreate, db: AsyncSession = Depends(get_db_dep), user=Depends(get_current_user)):
